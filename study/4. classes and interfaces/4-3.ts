...
// ❗️ 인터페이스 VS 추상클래스
// 인터페이스
// - 여러 개의 인터페이스를 implements하여 다양한 기능을 조합할 수 있지만 구현을 가지지는 않는다.
// - 인터페이스는 자바스크립트로 컴파일 되지 않아 파일 크기를 줄일 수 있음
// 추상클래스
// - 구체적은 구현 세부 사항을 제공할 수 있고 계층적인 클래스 구조를 만들 수 있지만 단일 상속만 가능하다.
// - 추상 클래스를 쓰면 자바스크립트에서 일반적인 클래스로 바뀐다. (= 즉 파일 크기가 좀 더 커지고, 추가 클래스가 만들어진다는 뜻)
